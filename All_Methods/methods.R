...
bayes_psps_horse = function(data){
  ...
  modres = sampling(stan_model_psps_horse, data = stan_data, chains = 1,
 	                  warmup = 1000, iter = 2000, thin = 1,
                    pars = c('pscore'), verbose = F)

  calc_weight = function(pscore, a)  (a*mean(data$A)/pscore) + ((1-a)*(1-mean(data$A))/(1-pscore))
  
  post_draws = rstan::extract(modres, pars = 'pscore')$pscore
  
  delta_hat = c()
  for (n in 1:nrow(post_draws)) {
    w1 = calc_weight(post_draws[n, ], avec)
    delta_hat[n] = lm( yvec ~ avec, weights = w1)$coefficients[2]
  }
  post_mean = mean(delta_hat)
  post_lwr = quantile(delta_hat, probs = .025)
  post_upr = quantile(delta_hat, probs = .975)
...
bayes_psps_lasso = function(data){
  ...
  modres = sampling(stan_model_psps_lasso, data = stan_data, chains = 1,
 	                  warmup = 1000, iter = 2000, thin = 1,
                    pars = c('pscore'), verbose = F)
...
bayes_psps_ridge = function(data){
  ...
  modres = sampling(stan_model_psps_ridge, data = stan_data, chains = 1,
     	              warmup = 1000, iter = 2000, thin = 1,
                    pars = c('pscore'), verbose = F)
